...
const AdminLogin = () => {
  const { setIsAdmin, navigate,axios } = useContext(AppContext);  // ✅ Correct destructuring
  ...
  const handleSubmit =  async (e) => {
    e.preventDefault();
     try {
      const {data} =  await axios.post(`${BASE_URL}/admin/login`, formData);
       if(data.success){
  ...
  return (
    <div className='flex items-center justify-center h-screen' style={{ backgroundColor: '#111e5c' }}>
      <form onSubmit={handleSubmit} className="max-w-sm w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
        ...
        <button
          type="submit"
          className="my-6 w-full h-11 rounded-full text-white transition-opacity" style={{ backgroundColor: '#111e5c' }} onMouseEnter={(e) => e.target.style.opacity = '0.9'} onMouseLeave={(e) => e.target.style.opacity = '1'}
        >
           Login
        </button>
      </form>
